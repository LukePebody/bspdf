...
@dataclass
class TateyokoPuzzle(Puzzle):
    """Tateyoko givens as ``(row, col, value, is_block)`` tuples."""
    clues: list = field(default_factory=list)
...
def decode_puzzle(url, title="", date=""):
    ...
    type_module = get_type_module(puzzle_type)
    if type_module is None:
        return UnknownPuzzle(puzzle_type=puzzle_type, cols=cols, rows=rows,
                             title=title, date=date, url=url)
    return type_module.decode(puzzle_type, cols, rows, data, title, date, url)
